...
/**
 * Daftar prioritas peresepan PKMK.
 *
 * ==========================================================================
 * TAHAP 2a: DAFTAR PRIORITAS DIBACA DARI SUPABASE
 *
 * Dahulu disaring dari `SAMPLE_BALITA_DATABASE` yang isinya `[]`, sehingga
 * dietisien selalu melihat daftar kosong dan panel peresepan tidak pernah dapat
 * dibuka. Itu pula sebabnya cacat P-1 — kebutuhan energi 770 kkal yang mati —
 * tidak pernah tertangkap saat dipakai: panelnya tidak pernah terbuka.
 * ==========================================================================
 */
export function PanelDietisien({
  daftarBalita,
  galatBaca,
}: {
  daftarBalita: BalitaDetail[]
  galatBaca: string | null
}) {
  ...
  const daftarPrioritas = daftarBalita.filter((b) => {
  ...
  return (
    <div className="space-y-6">
      {galatBaca && (
        <div className="rounded-2xl border-2 border-bahaya-garis bg-bahaya-bg p-4 text-xs text-bahaya-teks">
          <p className="font-bold">Daftar prioritas ini tidak lengkap.</p>
          <p className="mt-0.5 leading-relaxed">
            Data balita gagal dibaca dari server: {galatBaca} Daftar kosong di bawah BUKAN
            berarti tidak ada balita yang perlu PKMK. Muat ulang halaman ini.
          </p>
        </div>
      )}
          ...
          <div className="w-full max-w-4xl rounded-t-3xl bg-white p-6 shadow-2xl transition-all sm:rounded-3xl sm:p-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-5">
              <FormulasiPKMKSection
                ...
                // Kebutuhan energi berasal dari kolom `kalori_catchup_kkal`
                // pada hasil skrining balita ini, bukan angka literal. Bila
                // balita belum pernah diskrining, nilainya null dan panel
                // menyatakannya — tidak menebak. Lihat temuan audit P-1.
                targetEnergiDefaultKkal={
                  balitaTerpilih.riwayat[balitaTerpilih.riwayat.length - 1]
                    ?.kaloriCatchUpKkal ?? null
                }
                // Skrining yang menjadi DASAR peresepan disebutkan tegas,
                // supaya server membaca kebutuhan energi dari baris itu dan
                // bukan dari skrining terbaru yang mungkin dicatat kader
                // sementara panel ini terbuka. Lihat temuan audit P-3.
                skriningId={
                  balitaTerpilih.riwayat[balitaTerpilih.riwayat.length - 1]?.id
                }
