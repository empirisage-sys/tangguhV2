...
/**
 * Kalkulator kenaikan berat badan.
 *
 * ==========================================================================
 * TAHAP 2a: DAFTAR BALITA DIBACA DARI SUPABASE
 *
 * Dahulu memilih dari `SAMPLE_BALITA_DATABASE` yang isinya `[]`, sehingga mode
 * "pilih balita" selalu kosong dan hanya kalkulator manual yang dapat dipakai.
 * Akibatnya seluruh perbaikan velocity — termasuk pengurangan delta WHO dan
 * pelabelan sumber ambang — belum pernah terlihat pada data balita sungguhan.
 * ==========================================================================
 */
export function PanelVelocity({
  daftarBalita,
  galatBaca,
}: {
  ...
  const balita = useMemo(
    () => daftarBalita.find((b) => b.id === balitaTerpilihId),
    [daftarBalita, balitaTerpilihId],
  )
    ...
    <div className="space-y-6">
      {galatBaca && (
        <div className="rounded-2xl border-2 border-bahaya-garis bg-bahaya-bg p-4 text-xs text-bahaya-teks">
          <p className="font-bold">Daftar balita tidak dapat dibaca.</p>
          <p className="mt-0.5 leading-relaxed">
            {galatBaca} Pilihan balita di bawah BUKAN berarti kosong. Kalkulator manual
            tetap dapat dipakai sementara.
          </p>
        </div>
      )}
      {/* Header Banner */}
